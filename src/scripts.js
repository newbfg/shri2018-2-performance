...
rangeSLider.oninput = () => output.innerHTML = this.value > 0 ? '+' + this.value : this.value;
...
const scrolling = (el, val) => el.scroll({top: 0, left: val, behavior: 'smooth'});
...
const rotateToValue = rotate => Math.floor((Math.abs(rotate * 360 * 1.73 + 265) / 53) + 26);
...
document.querySelectorAll('.panel_temp').forEach(p => p.onclick = () => showModal('.modal_temp'));
document.querySelectorAll('.panel_lamp').forEach(p => p.onclick = () => showModal('.modal_light'));
document.querySelectorAll('.panel_floor').forEach(p => p.onclick = () => showModal('.modal_knob'));
...
selectButton.addEventListener('click', () => popup.classList.toggle('filter__select-popup_open'));

selectOptions.forEach(o => o.addEventListener('click', e => {
